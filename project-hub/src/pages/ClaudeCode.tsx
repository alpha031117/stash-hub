...
// Links point at relative brain paths that aren't navigable here, so we render
// the label text only.
...
// Lightweight markdown renderer for the brain/auto-memory content. Not a full
// CommonMark engine — handles headings (rendered as bold, not <h1>), bold,
// inline code, fenced code, bullet/ordered lists, rules, and leading
// frontmatter. Tables and nested lists are flattened.
function Markdown({ text }: { text: string }) {
  ...
  // Leading YAML frontmatter → a dimmed key/value block instead of stray '---'.
  if (lines[0]?.trim() === '---') {
  ...
  while (i < lines.length) {
    ...
    // Heading → bold (per request: '#' becomes bold, not an <h1>).
